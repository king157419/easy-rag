...
import requests
from config import MAX_NEW_TOKENS_GEN, TEMPERATURE, TOP_P, DEEPSEEK_API_KEY, DEEPSEEK_BASE_URL

def generate_answer(query, context_docs, gen_model, tokenizer):
    """Generates an answer using DeepSeek API based on query and context."""
    if not context_docs:
...
""")
    ...
    # 改进的提示词工程
    system_prompt = """你是一位专业的中医医疗问答助手，擅长根据医疗文献回答用户问题。
...
现在请回答："""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"DeepSeek API error: {response.status_code} - {error_msg}")
            return f"抱歉，生成答案时遇到错误（HTTP {response.status_code}），请稍后重试。"
    except requests.exceptions.Timeout:
        st.error("请求超时")
        return "抱歉，生成答案超时，请稍后重试。"
    except requests.exceptions.ConnectionError:
        st.error("连接DeepSeek服务失败")
        return "抱歉，无法连接到DeepSeek服务，请检查网络连接。"
